...
class PoolForm(forms.Form):
    ...
    rating_design = forms.IntegerField(label='Оценка дизайна (1-5)', min_value=1, max_value=5)
    rating_content = forms.IntegerField(label='Оценка контента (1-5)', min_value=1, max_value=5)
    features_liked = forms.CharField(label='Какие функции вам понравились?', widget=forms.Textarea)
